...
test("should be defined Event", () => {
  const event = createEvent();

  expect(event).toBeDefined();
});

test("`.watch`: вызывает функцию с сайд-эффектами при каждом срабатывании события", () => {
...
test("`.map`: создает производное событие на основе данных из исходного", () => {
  ...
  const userNameUpdatedWatcherMock = vi.fn();
  const userRoleUpdatedWatcherMock = vi.fn();
...
test("`.prepend`: создает событие-триггер для преобразования данных перед запуском исходного эвента", () => {
  const watcherMock = vi.fn();
...
test("`.filterMap`: создает производное событие на основе данных из исходного с возможностью отмены вызова", () => {
...
test("`.filter`: cоздает производное событие с возможностью отмены вызова", () => {
  const numbers = createEvent<{ x: number }>();
  const positiveNumbers = numbers.filter({ fn: ({ x }) => x > 0 });
  const numberWatcherMock = vi.fn();
  const watcherMock = vi.fn();
